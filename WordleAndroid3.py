...
class WordleHelperApp(App):
    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        # Set window size after app is initialized but before build
        if hasattr(Window, 'size') and sys.platform != 'android':
            Window.size = (400, 700)
    ...
    def build(self):
        ...
        # Best starting word recommendation
        best_word_label = Label(
            text=f"Optimal starting word: '{self.solver.best_starting_word.upper()}'",
            size_hint_y=0.05,
            font_size=16,
            color=(0.2, 0.5, 0.2, 1)
        )
        layout.add_widget(best_word_label)
        
        # Use best word button
        best_word_btn = Button(
            text=f"Use '{self.solver.best_starting_word.upper()}'",
            size_hint_y=0.07,
            background_color=(0.2, 0.6, 0.2, 1),
            on_press=self.use_best_word
        )
        layout.add_widget(best_word_btn)
        
        # Word input
        input_layout = BoxLayout(orientation='horizontal', size_hint_y=0.09, spacing=10)
        input_layout.add_widget(Label(text="Your guess:", size_hint_x=0.3, color=(0, 0, 0, 1)))
        
        self.word_input = TextInput(
            multiline=False, 
            size_hint_x=0.5,
            hint_text="Enter 5-letter word",
            on_text_validate=self.on_guess_entered,
            background_color=(1, 1, 1, 1),
            foreground_color=(0, 0, 0, 1),
            write_tab=False  # Prevent tab issues on mobile
        )
        ...
        self.submit_btn = Button(
            text="Submit", 
            size_hint_x=0.2, 
            on_press=self.on_guess_entered,
            background_color=(0.2, 0.4, 0.8, 1)
        )
        input_layout.add_widget(self.submit_btn)
        
        layout.add_widget(input_layout)
        
        # Feedback buttons
        self.feedback_buttons = []
        feedback_layout = GridLayout(cols=5, size_hint_y=0.14, spacing=5)
        for i in range(5):
            btn = FeedbackButton(
                position=i, 
                size_hint=(None, None), 
        ...
        legend_layout.add_widget(Label(text="X: Not in word", size_hint_x=0.33, color=(0.5, 0.5, 0.5, 1)))
        legend_layout.add_widget(Label(text="Y: Wrong position", size_hint_x=0.34, color=(0.8, 0.8, 0, 1)))
        legend_layout.add_widget(Label(text="G: Correct", size_hint_x=0.33, color=(0, 0.8, 0, 1)))
        layout.add_widget(legend_layout)
        
        # Action buttons
        action_layout = BoxLayout(orientation='horizontal', size_hint_y=0.09, spacing=10)
        
        self.suggest_btn = Button(
            text="Find Best Word", 
            on_press=self.get_suggestions,
        ...
        self.status_label = Label(
            text=f"Start with '{self.solver.best_starting_word.upper()}' or enter your own guess", 
            size_hint_y=0.05,
            color=(0.3, 0.3, 0.3, 1)
        )
    ...
    def on_start(self):
        """Called when the app starts - safe to set window size here"""
        if hasattr(Window, 'size') and sys.platform != 'android':
            Window.size = (400, 700)
        
        # Add the initial best word to history when app starts
    ...
    def use_best_word(self, instance):
        """Prefill the input with the best starting word"""
        self.word_input.text = self.solver.best_starting_word
        self.status_label.text = f"Using optimal word: '{self.solver.best_starting_word.upper()}'. Set feedback and press Submit."
    
    def use_suggestion(self, word):
        """Use a suggestion word in the input field"""
        self.word_input.text = word
        self.status_label.text = f"Using suggestion: '{word.upper()}'. Set feedback and press Submit."
    
    def on_guess_entered(self, instance):
        guess = self.word_input.text.strip().lower()
        if len(guess) != 5 or not guess.isalpha():
            self.status_label.text = "Please enter a valid 5-letter word"
            return
        ...
        # Clear input and reset feedback buttons
    ...
    def calculate_suggestions(self):
        ...
        # Add new suggestions with highlighting for the best word
        for i, word in enumerate(suggestions):
            if i == 0:  # Best word (lowest score)
                label = ClickableSuggestionLabel(
                    app_instance=self,
                    word=word,
                    is_best=True,
                    text=f"★ {word.upper()} ★ (Best - Click to use)", 
                    size_hint_y=None, 
                    height=40,
                    font_size=18,
                    color=(0, 0.6, 0, 1),
                ...
                with label.canvas.before:
                    Color(0.9, 1, 0.9, 1)
                    Rectangle(pos=label.pos, size=(Window.width, 40))
            else:
                label = ClickableSuggestionLabel(
                    app_instance=self,
                    word=word,
        ...
        self.status_label.text = f"Found {len(suggestions)} suggestions. {len(self.solver.possible_words)} words possible."
    ...
    def reset_solver(self, instance):
        ...
        if success:
            ...
            self.solver.guess_history.append((self.solver.best_starting_word.upper(), 'xxxxx'))
            self.update_history_display()
            self.status_label.text = "Solver reset. Start with 'ROATE' or enter your own guess."
        else:
