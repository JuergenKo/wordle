...
class WordleHelperApp(MDApp):
    ...
    def on_start(self):
        ...
        if self.ads:
            try:
                self.ads.init()
                self.ads.new_banner(self.banner_ad_id, top_pos=False)  # bottom banner
                self.ads.request_banner()  # fetch ad from AdMob
                self.ads.show_banner()     # display banner
                Logger.info("KivMob: Banner requested and showing...")
            except Exception as e:
