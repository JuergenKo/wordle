...
class WordleHelperApp(App):
    ...
    def fix_window_size(self, dt):
        print("Attempting window fix...")
        Window.fullscreen = 'auto'
        Window.size = Window.system_size
        print(f"After fix - Size: {Window.size}, Fullscreen: {Window.fullscreen}")
        
        # Try additional fixes
        self.try_alternative_fixes()
    
    def try_alternative_fixes(self):
        """Try multiple approaches"""
        try:
            # Method 1: Direct size setting
            Window.size = (Window.system_size[0], Window.system_size[1])
            print("Method 1 applied")
            
            # Method 2: Use 'fake' fullscreen
            Window.fullscreen = 'fake'
            print("Method 2 applied")
            
            # Method 3: Schedule another attempt
            Clock.schedule_once(self.force_fullscreen, 1.0)
            
        except Exception as e:
            print(f"Error in window fix: {e}")
    
    def force_fullscreen(self, dt):
        """Final attempt"""
        Window.fullscreen = 'auto'
        print("Final fullscreen attempt")
