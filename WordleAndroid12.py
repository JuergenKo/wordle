...
class WordleHelperApp(App):
    ...
    def fix_window_size(self, dt):
        """Force proper fullscreen on Honor devices"""
        try:
            from android.runnable import run_on_ui_thread
            from jnius import autoclass
            
            @run_on_ui_thread
            def set_immersive_mode():
                try:
                    PythonActivity = autoclass('org.kivy.android.PythonActivity')
                    activity = PythonActivity.mActivity
                    View = autoclass('android.view.View')
                    
                    # Set immersive sticky mode (hides nav bar completely)
                    ...
                    print(f"Immersive mode error: {e}")
            
            set_immersive_mode()
            
            # Also set Kivy window properties
            Window.fullscreen = 'auto'
            Window.size = Window.system_size
            
            print(f"Fixed window: {Window.size}, System: {Window.system_size}")
