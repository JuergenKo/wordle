...
class WordleHelperApp(App):
    ...
    def fix_window_size(self, dt):
        """Force true fullscreen on Honor devices"""
        try:
            from android.runnable import run_on_ui_thread
            from jnius import autoclass
            
            @run_on_ui_thread
            def set_true_fullscreen():
                try:
                    ...
                    # For Honor specifically, try to force the window size
                    window.setAttributes(window.getAttributes())
                    
                except Exception as e:
                    print(f"True fullscreen error: {e}")
            
            set_true_fullscreen()
            
            # Also force Kivy window to system size
            Window.fullscreen = 'auto'
            Window.size = Window.system_size
            
            # Schedule multiple attempts (Honor needs this)
            Clock.schedule_once(lambda dt: setattr(Window, 'size', Window.system_size), 0.3)
            Clock.schedule_once(lambda dt: setattr(Window, 'fullscreen', 'auto'), 0.5)
            Clock.schedule_once(lambda dt: setattr(Window, 'size', Window.system_size), 1.0)
            
            print(f"Window fix attempted: {Window.size} vs {Window.system_size}")
            
        except Exception as e:
            print(f"Window fix failed: {e}")
    ...
    def on_start(self):
        """Fix window size for Android or desktop - MOVED FROM build()"""
        try:
            if sys.platform == 'android':
    ...
    def fix_android_window(self, dt):
        ...
        try:
            ...
            # Additional Honor-specific fix
            self.honor_specific_fix()
            
        except Exception as e:
            print(f"Android window fix error: {e}")
    
    def honor_specific_fix(self):
        """Extra fixes for Honor/Huawei devices"""
        try:
            # Try multiple approaches for Honor
            Clock.schedule_once(lambda dt: setattr(Window, 'fullscreen', 'auto'), 0.1)
            Clock.schedule_once(lambda dt: setattr(Window, 'size', Window.system_size), 0.2)
            
            # Force redraw
            Clock.schedule_once(lambda dt: Window.dispatch('on_resize', *Window.size), 0.3)
            
        except Exception as e:
            print(f"Honor fix error: {e}")    
