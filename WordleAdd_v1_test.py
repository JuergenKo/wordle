...
from kivy.logger import Logger # <-- ADD FOR DEBUGGING
...
class WordleHelperApp(App):
    ...
    def build(self):
        ...
        # Top part: your app content
        content = Label(text="Your Wordle Game Goes Here", size_hint_y=0.9)
        layout.add_widget(content)
        
        # Bottom part: space for the banner
        banner_placeholder = BoxLayout(size_hint_y=0.1) # Reserve 10% height for the banner
        layout.add_widget(banner_placeholder)
        
        return layout

    def on_start(self):
        self.ads.new_banner(self.banner_ad_id)
        # Try different positions if it's not showing
        # self.ads.show_banner(top=False) # Show at bottom
        self.ads.show_banner()    # Show at top
